...
class Coach:
	...
	def run(self):
		...
		for ep in range(0, args.epoch):#记录最优
	...
	def trainEpoch(self):
		...
		for i, batch in enumerate(diffusionLoader):
			...
			uEmbeds = self.model.getUserEmbeds().detach()#获取用户实体嵌入
			...
			loss = diff_loss.mean() * (1-args.e_loss) + ukgc_loss.mean() * args.e_loss#结合权重计算扩散模型损失和用户知识图谱损失
		...
		with torch.no_grad():
			...
			for _, batch in enumerate(diffusionLoader):
				batch_item, batch_index = batch
				batch_item, batch_index = batch_item.cuda(), batch_index.cuda()#遍历 diffusionLoader，获取去噪后的批次数据。
				denoised_batch = self.diffusion_model.p_sample(self.denoise_model, batch_item, args.sampling_steps)#p_sample 方法生成去噪后的嵌入，
				top_item, indices_ = torch.topk(denoised_batch, k=args.rebuild_k)#并获取前 k 个项
				for i in range(batch_index.shape[0]):
					for j in range(indices_[i].shape[0]):
						h_list.append(batch_index[i])
						t_list.append(indices_[i][j])#记录头实体和尾实体的列表

			edge_set = set()
			for index in range(len(h_list)):
				edge_set.add((int(h_list[index].cpu().numpy()), int(t_list[index].cpu().numpy())))#创建边集，确保每个边的唯一性，并补充反向边。
			for index in range(len(h_list)):
			...
			for index in range(len(h_list)):
				...
				except Exception:#根据头实体和尾实体构建去噪后的知识图谱边。
					continue
		...
		with torch.no_grad():
			...
			self.generatedKG = denoisedKG#通过随机掩码选择去噪后的知识图谱边，生成最终的知识图谱。

		for i, tem in enumerate(trnLoader):
			ancs, poss, negs = tem
			ancs = ancs.long().cuda()
			poss = poss.long().cuda()
			negs = negs.long().cuda()#锚点和正负样本
			...
			regLoss = calcRegLoss(self.model) * args.reg#bpr损失和正则化损失
			...
			denoisedKGEmbeds = torch.concat([usrEmbeds, itmEmbeds], axis=0)#pj0
